...
.DESCRIPTION
    Returns information about the Git repository state
    including branch, commit, and working tree status.
.EXAMPLE
...
.DESCRIPTION
    Returns the last N commits in the repository.
.PARAMETER Count
    Number of commits to return (default: 10).
.EXAMPLE
    $history = Get-GitCommitHistory -Count 5
#>
...
.PARAMETER Message
    The commit message.
.PARAMETER StageAll
    If true, stages all changes (git add -A).
.EXAMPLE
    New-GitCommit -Message "Updated repository baseline"
#>
...
.SYNOPSIS
    Creates a Git tag.
.DESCRIPTION
    Creates a tag with the specified name and optional message.
.PARAMETER TagName
    The name of the tag.
.PARAMETER Message
    Optional tag message.
.EXAMPLE
    New-GitTag -TagName "Repository.Common-v1.9"
#>
function New-GitTag {
    [CmdletBinding()]
    param(
...
<#
.SYNOPSIS
    Gets all Git branches.
.DESCRIPTION
    Lists all branches with current branch indicator.
.EXAMPLE
...
.EXAMPLE
    Invoke-GitPush -Remote origin -Branch main
#>
...
.DESCRIPTION
    Pulls the current branch from the remote repository.
.PARAMETER Remote
    The remote name (default: origin).
.PARAMETER Branch
    The branch to pull (default: current branch).
.EXAMPLE
    Invoke-GitPull -Remote origin -Branch main
#>
...
Export-ModuleMember -Function @(
    ...
    # Git Service (New — Phase 4) — 7 functions
    'Get-GitStatus',
